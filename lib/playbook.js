 ...
 */
export function build(stock, { profile, dataAge, event, triggerPrice, direction = "buy" } = {}) {
  ...
  const stopZ = stopZones.find(z => z.convergence >= 1) || stopZones[0] || null;
  ...
  const stopPct = stopZ ? round2(pct(basisPrice, stopZ.mid)) : round2(-(atrValue * 1.5 / basisPrice) * 100);
  ...
  const exits = {
    ...
    actionLabel: isSell ? "Sell at" : "Entry",
    targetLabel: isSell ? "Buy back" : "Target",
    ...
    stop: stopZ
      ...
      : { zone: { low: round2(basisPrice * (1 + stopPct / 100)), high: round2(basisPrice) }, pct: stopPct,
  ...
  return {
    ...
    atr: lv.atr,
    entry: {
      label: isSell ? "Sell at" : "Entry",
      kind: isSell ? "trim level" : (triggered ? "pullback entry" : "breakout trigger"),
