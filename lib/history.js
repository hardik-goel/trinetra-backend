...
export function recordSignal({ symbol, name, price, groups, evaluation, at, profileId, horizon, potential, confidence, exits, dataAge, direction = "buy" }) {
...
const isLeakage = r => {
  if (!HOLDINGS_PROFILES.has(r.profileId)) return false;
  ...
  if (r.profileId === "sell_holdings") return r.direction !== "sell";
  return (r.firedAt ?? 0) < APPLIES_TO_ENFORCED_AT;
};
...
export function stats(days = 30) {
  ...
  return {
    ...
    excluded: leaked.length
      ? { n: leaked.length,
          reason: "screener signals filed under a holdings-only profile, from before appliesTo was enforced on 2026-08-03 — fired across the whole universe rather than on stock you held, so they say nothing about the sell/buy-back cycle" }
      : null,
