...
/* Shared with /signals/history rather than reimplemented: the brief and the track
   record must reach the same verdict on the same record, and two copies of this
   rule would drift. */
import { guardLegacyLevels } from "./history.js";
...
export function build({ signals, holdings, exitSignals, events, concentration, ipos, dataHealth, profiles, now = Date.now() }) {
  ...
  for (const id of Object.keys(byProfile)) {
    byProfile[id].sort((a, b) =>
      ...
      (b.volX ?? 0) - (a.volX ?? 0));
    byProfile[id] = byProfile[id].map(s => guardLegacyLevels({
      ...s,
...
export function renderTelegram(brief) {
  ...
  if (brief.ipos?.length) {
    L.push(`\n<b>IPOs closing</b>`);
    for (const i of brief.ipos.slice(0, 5)) {
      L.push(`• ${esc(i.name)}${i.segment ? ` (${esc(i.segment.toLowerCase())})` : ""} — <b>${esc(i.verdict || "no call")}</b>${i.closingTomorrow ? " · closes tomorrow" : ` · closes ${esc(i.closeDate || "soon")}`}`);
      if (i.oneLiner) L.push(`  <i>${esc(i.oneLiner)}</i>`);
    }
  }
