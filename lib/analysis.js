...
export function resistanceAbove(stock, candles) {
  ...
  // Round numbers act as resistance in practice; the nearest one above.
...
export function findAnalogs(candles, { lookback = 20, volMult = 1.5, sessions = 5 } = {}) {
  if (!candles || candles.length < lookback + sessions + 30) return { n: 0, mfe: [], mae: [] };
  const mfe = [], mae = [];
  for (let i = lookback; i < candles.length - sessions; i++) {
    ...
    mfe.push(pct(c.c, Math.max(...fwd.map(x => x.h))));
...
export function potential(stock, { horizon, triggerPrice }) {
  ...
  const resistance = resistanceAbove(stock, candles);
  const lookback = horizon === "positional" ? 50 : 20;
  const { n, mfe, mae } = findAnalogs(candles, { lookback, sessions });
  ...
  const resistCapPct = resistance ? round2(pct(price, resistance.price)) : null;
  ...
  // Cap against what volatility and structure actually permit, and record which
  // one bit — an uncapped estimate that ignores a wall is not an estimate.
  let cappedBy = null;
  if (atrCap != null && high > atrCap) { high = atrCap; cappedBy = "atr"; }
  if (resistCapPct != null && high > resistCapPct) {
    high = round2(resistCapPct);
    cappedBy = `resistance:${resistance.name}@${round2(resistance.price)}`;
  }
  ...
  const remaining = v => round2(Math.max(0, v - movedAlreadyPct));
...
export function exitLevels(stock, { pot, conf, atr }) {
  ...
  if (pot.converged) {
    ...
    return {
      converged: true,
      safe: at, primary: at, stretch: at, stop: stop0,
      riskReward: { toSafe: rr0, toPrimary: rr0, toStretch: rr0 },
      riskRewardWarning: rr0 != null && rr0 < 1 ? "Risk-reward to the only available target is below 1:1." : null,
  ...
  const primary = lvl(pot.estRangePct.median,
    `The median outcome of those setups${pot.resistance ? `, kept below ${pot.resistance.level} at ₹${pot.resistance.price}, which has rejected price before` : ""}.`);
  ...
  return {
    ...
    riskRewardWarning: riskReward.toPrimary != null && riskReward.toPrimary < 1
      ? "Risk-reward to the primary target is below 1:1." : null,
    suggestion,
  };
}
