...
export async function notifyCycle(tg, sig) {
  ...
  /* Direction-aware pricing, in the vocabulary of the action being suggested.
     "Target ₹9389" on a trim would read as something to hope for rather than the
     level to buy back at, and a fall shown as a negative percentage reads as a
     loss. So: magnitude, an arrow, and the word for the side. */
  const pr = sig.pricing;
  if (pr) {
    L.push(`<b>${pr.actionLabel}</b>${" ".repeat(Math.max(1, 14 - pr.actionLabel.length))}₹${pr.actionPrice}`);
    L.push(`<b>${pr.targetLabel}</b>${" ".repeat(Math.max(1, 14 - pr.targetLabel.length))}₹${pr.targetPrice}  (${pr.movePct}% ${pr.downward ? "below" : "above"})`);
    L.push(`<b>Potential</b>     ${pr.movePct}% ${pr.arrow}`);
    if (pr.stop) L.push(`<b>Invalidated</b>   ${pr.stop.above ? "above" : "below"} ₹${pr.stop.price}`);
    if (pr.confidence) L.push(`<b>Confidence</b>    ${pr.confidence.score}% (${pr.confidence.band})`);
    L.push("");
  }
