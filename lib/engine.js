...
export function evaluate(s, criteria, { requireAll = false } = {}) {
  ...
  /* Computed once and used for both the flag and the sentence. They disagreed:
     the flag required every counted criterion to pass, while the sentence fired
     on any row with missing data — so a row with a GENUINE failure was told "it
     would otherwise have locked", which is simply false. Missing data and a
     failed criterion are different reasons for silence, and conflating them is
     the exact thing the withheld state exists to prevent. */
  const allCountedPass = counted.length > 0 && counted.every(r => r.pass);
  const withheld = !!(requireAll && skipped.length > 0 && allCountedPass);
  ...
  return {
    ...
    locked: allCountedPass && !(requireAll && skipped.length > 0),
    ...
    withheldForMissingData: withheld,
    ...
    /* Structured alongside the prose. A dashboard that aggregates these across 300
       stocks gets seven identical-looking sentences with no symbol and no way to
       dismiss one — dismissing a string breaks the moment the wording changes.
       Each entry carries a stable code so it can be grouped, counted and
       dismissed durably, and the text stays for reading. */
    warningsDetail: [
      ...(withheld ? [{
        code: "withheld_missing_data",
        criterion: skipped.map(r => r.name).join(", "),
        metrics: skipped.flatMap(r => (r.skippedChecks || []).map(x => x.metric)),
        severity: "info",
      }] : []),
      ...skipped.map(r => ({
        code: "criterion_no_data",
        criterion: r.name,
        metrics: (r.skippedChecks || []).map(x => x.metric),
        severity: "warn",
      })),
      ...results.filter(r => r.partial).map(r => ({
        code: "criterion_partial",
        criterion: r.name,
        metrics: (r.skippedChecks || []).map(x => x.metric),
        judged: r.checksOut.length - r.skippedChecks.length,
        of: r.checksOut.length,
        severity: "warn",
      })),
    ],
    warnings: [
