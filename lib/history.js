...
export function recordSignal({ symbol, name, price, groups, evaluation, at, profileId, profileName, horizon, potential, confidence, exits, dataAge, direction = "buy", entryPrice, exitPrice, stopPrice, levelSource, noRoom, riskRewardWarning }) {
  ...
  const rec = {
    ...
    /* `noRoom` rides on the percentage block too. A reader holding only
       exitLevels — which is what the brief renders — could otherwise see three
       nulls and no reason for them, which is the state this whole field exists
       to distinguish from "never computed". */
    exitLevels: exits ? { safe: exits.safe?.pct ?? null, primary: exits.primary?.pct ?? null,
                          stretch: exits.stretch?.pct ?? null, stop: exits.stop?.pct ?? null,
                          noRoom: !!exits.noRoom } : null,
    /* The actual rupee levels the alert carried. Percentages alone cannot be
       checked against the message the user received, and "the alert said sell at
       X" is the thing a track record has to be able to answer. */
    /* `noRoom` says WHY exit is null, and the track record cannot reconstruct it
       later: a rejected target and a record written before the field existed both
       store exit: null, and reading entry-and-stop-with-no-exit as "no target
       sent" is a guess about a shape rather than a fact about the signal. The
       reason is stored at fire time or it is not recoverable. */
    levels: { entry: entryPrice ?? null, exit: exitPrice ?? null, stop: stopPrice ?? null,
              source: levelSource ?? null, noRoom: !!noRoom,
              riskRewardWarning: riskRewardWarning ?? null },
    ...
    /* Stored, not recomputed at display time. Whether a lock cleared every
       criterion or only the ones with data is a property of the moment it fired,
       and the data that made it partial may be present by the time anyone looks.
       A hit rate blended across three-criteria and two-criteria locks describes a
       strategy that was never run, and that cannot be repaired after the fact.

       Falls back to deriving from the persisted criteria rather than writing null
       when an evaluation is absent — `skipped` is kept on every criterion now, so
       the answer is present in the record either way. */
    lockQuality: evaluation?.lockQuality
      ?? (criteria.some(c => c.skipped) ? "partial" : "full"),
    requireAll: !!evaluation?.requireAll,
    notEvaluated: evaluation?.notEvaluated ?? criteria.filter(c => c.skipped).map(c => c.name),
...
/* Records written before the risk-reward guard carry levels the engine would now
   refuse to emit: HINDCOPPER stored {safe 0.74, primary 0.74, stretch 0.74, stop
   -4.38} — a 0.17:1 trade — and it rendered in the morning brief beside an "I took
   this" button. The record itself is evidence and is never rewritten; what it must
   not do is present a rejected level as advice, so the ratio is recomputed on the
   way out and the targets are withheld exactly as a fresh signal would be.

   Applied ONLY where the field is absent. A record that already carries
   `levels.noRoom` was judged at fire time and that judgement stands — re-deriving
   it here would overrule what was actually true when the alert went out. */
export function guardLegacyLevels(r) {
  if (r?.levels && Object.prototype.hasOwnProperty.call(r.levels, "noRoom")) return r;
  const el = r?.exitLevels;
  const stop = Math.abs(el?.stop ?? 0);
  const best = Math.max(el?.stretch ?? -Infinity, el?.primary ?? -Infinity, el?.safe ?? -Infinity);
  if (!(stop > 0) || !Number.isFinite(best) || best / stop >= 1) return r;
  /* Rounded to two places, a ratio of 0.998 prints as "1:1" — a row withheld for
     being below 1:1 while stating it is exactly 1:1. Three places when the round
     would otherwise contradict the reason. */
  const raw = best / stop;
  const rr = round2(raw) >= 1 ? Math.round(raw * 1000) / 1000 : round2(raw);
  return {
    ...r,
    exitLevels: { ...el, safe: null, primary: null, stretch: null },
    noRoom: true,
    levelsWithheldOnRead: {
      why: "written before the risk-reward guard existed",
      riskReward: rr,
      original: { safe: el?.safe ?? null, primary: el?.primary ?? null, stretch: el?.stretch ?? null },
    },
    riskRewardWarning: `Even the best-case target was ${rr}:1 against the stop. No target is shown — this signal fired before the check existed, and the engine would not emit these levels today.`,
  };
}
...
export function list({ from, to } = {}) {
  ...
  return history.filter(r => r.firedAt >= lo && r.firedAt < hi).map(guardLegacyLevels);
}
...
export function stats(days = 30) {
  ...
  return statsOf(history.filter(r => r.firedAt >= since), days);
}

/* Split from `stats` so the same aggregation serves the file store and a user's
   rows from Postgres. Two implementations would drift, and the one that drifts
   is the one nobody is looking at — which here means one user's track record
   silently computed by different rules from another's. */
export function statsOf(inWindow, days = 30) {
