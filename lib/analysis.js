...
export function potential(stock, { horizon, triggerPrice }) {
  ...
  return {
    ...
    cappedBy, converged,
    /* The 75th percentile of the drawdown these setups inflicted, over the SAME
       window the targets are measured across. A stop set here survives three
       comparable setups in four; the median would stop you out of half of them.
       This exists because the stop was sized at 1.5× the DAILY ATR regardless of
       horizon while the target was capped at 1.5×ATR×√sessions — so on intraday,
       where √1 = 1, the ceiling on the target was exactly the floor on the stop
       and no intraday signal could ever clear 1:1. Every one of them came out
       noRoom. That was arithmetic, not a market fact. */
    analogs: { n, medianMFE: round2(median(mfe)), medianMAE, winRate,
               maeP75: round2(percentile([...mae].map(Math.abs).sort((a, b) => a - b), 0.75)),
...
export function exitLevels(stock, { pot, conf, atr }) {
  ...
  /* Sized on the same horizon as the target.
     `a * 1.5` is a DAILY range; using it as a floor put a full day's volatility
     behind a one-session trade, wider than the entire move that trade could make.
     The drawdown these setups actually inflicted over this window is the honest
     measure, taken at the 75th percentile so three in four would have survived it.
     Volatility still bounds it — scaled by the horizon, the same √sessions the
     target ceiling uses, so the two are finally on one time scale. */
  /* The MEDIAN drawdown, not the 75th percentile. The target is the median
     favourable move, so the stop must be the median adverse one — pairing a tail
     against a central value is the same error that made every target look
     unaffordable, just pointed the other way. Measured with p75 the swing stops
     widened by half and the risk-reward got worse, not better.
     Both bounds scale with √sessions so the floor is no longer a daily number
     policing a one-session trade. */
  const hz = Math.sqrt(pot.sessions || 1);
  /* An ABSOLUTE floor as well as a relative one. Scaling everything off ATR fails
     exactly where it matters: on a stock whose ATR is near zero the relative floor
     is also near zero, and the run produced 0.05% stops — hit by the spread, not
     by the idea being wrong. A stop that cannot survive a normal tick is not risk
     control, so 0.5% is the hard minimum whatever the volatility says. */
  const ABS_MIN_STOP_PCT = 0.5;
  const volFloor = Math.max(ABS_MIN_STOP_PCT, a ? a * 0.5 * hz : 0);
  const volCeil = a ? Math.max(volFloor, a * 1.5 * hz) : null;
  const measured = Math.abs(pot.analogs?.medianMAE ?? NaN);
  const stopPct = (() => {
    if (!a) return -3;
    if (!Number.isFinite(measured)) return -round2(Math.max(volFloor, a * 1.5));
    return -round2(Math.min(volCeil, Math.max(volFloor, measured)));
  })();
  ...
  if (pot.converged) {
    ...
    return {
      converged: true, noRoom: noRoom0,
      ...(noRoom0 ? { withheld: { primary: at.price, why: "below 1:1 against the stop" } } : {}),
  ...
  return {
    // Stated on both branches. An absent field and a false one are different
    // answers to a caller that binds it, and this shape must give only one.
    noRoom: false,
