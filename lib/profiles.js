...
/* The four the app was commissioned against, in the user's own words. These
   phrases are the contract: the display reads them from here, so the panel can
   never drift from what the engine actually runs.

   The fourth is DORMANT, not broken. Order-book depth is paid exchange data, so
   on a delayed feed it has nothing to answer with — it is excluded from the lock
   denominator rather than failing, which is why the header reads 3/3 and not
   3/4 forever. */
export const ORIGINAL_FOUR = [
  { id: "fund", originalIndex: 1, originalPhrase: "Fundamentally strong" },
  { id: "brk",  originalIndex: 2, originalPhrase: "Breakout" },
  { id: "vol",  originalIndex: 3, originalPhrase: "Volume shockers" },
  { id: "flow", originalIndex: 4, originalPhrase: "Buyers and sellers count and percentage",
    dormant: true, dormantReason: "order-book depth is paid exchange data — unavailable on the free delayed feed",
    requiresProvider: "kite" },
];
export const ORIGINAL_BY_ID = Object.fromEntries(ORIGINAL_FOUR.map(o => [o.id, o]));

/** The founding four with their live state, for the panel that pins them. */
export function originalFourStatus(criteria = [], provider = "yahooDelayed") {
  const byId = Object.fromEntries((criteria || []).map(c => [c.id, c]));
  return ORIGINAL_FOUR.map(o => {
    const c = byId[o.id];
    const live = provider === "kite";
    const status = !c ? "not in this profile"
      : o.dormant && !live ? "awaiting live data (Kite)"
      : c.enabled ? "active" : "disabled by you";
    return {
      ...o, status,
      present: !!c,
      checks: c?.checks ?? (o.id === "flow" ? [{ metric: "buyerPct", op: "gte", value: 60 }] : []),
      name: c?.name ?? null,
      countsTowardLock: status === "active",
    };
  });
}
...
export const CANONICAL_CRITERIA = [
  { id: "fund", key: "F", name: "Fundamentals", enabled: true, builtin: true,
    originalIndex: 1, originalPhrase: "Fundamentally strong",
    checks: [
  ...
  { id: "brk", key: "B", name: "Breakout", enabled: true, builtin: true,
    originalIndex: 2, originalPhrase: "Breakout",
    checks: [
  ...
  { id: "vol", key: "V", name: "Volume shocker", enabled: true, builtin: true,
    originalIndex: 3, originalPhrase: "Volume shockers",
    checks: [{ metric: "volMultiple", op: "gte", value: 3 }] },
  /* The fourth ships PRESENT and disabled rather than absent, so it is visibly
     dormant in the panel and becomes a one-tap enable the day Kite is connected —
     rather than a criterion the user has to remember existed. Disabled entries do
     not affect matchesCanonical, which compares the enabled set. */
  { id: "flow", key: "O", name: "Order flow", enabled: false, builtin: true, depthNote: true,
    originalIndex: 4, originalPhrase: "Buyers and sellers count and percentage",
...
/* Shorting, across the whole universe.

   This was previously out of scope on the grounds that a SELL on an unheld stock
   is a short recommendation. It is now in scope by explicit request, so the
   constraint moves from "we do not emit these" to "we emit these and state what
   they cost". Two things travel with every short and are not optional:

   1. **Loss is unbounded.** A long can go to zero; a short has no ceiling. The
      stop is not a nicety here, it is the entire risk control.
   2. **Most stocks cannot be shorted overnight.** In the cash market a short must
      be closed the same session. Only F&O stocks can be carried, in whole lots,
      through futures or options. lib/fno.js resolves this per symbol and the
      horizon is clamped to intraday when the stock is not F&O — a "3–5 day short"
      on a cash-only stock is not a trade that can be placed.

   The criteria are bearish in their own right, NOT inverted bullish ones. A stock
   failing a breakout test is not thereby breaking down, and treating the absence
   of strength as evidence of weakness is how a screener manufactures signals. */
export const SHORT_PROFILE = {
  short: {
    name: "Short",
    horizon: "swing",
    direction: "sell",
    enabled: false,   // ships off; shorting is opt-in, never a default
    // Alerts on once the profile is enabled: a short that only appears when the
    // dashboard happens to be open is a short you find out about too late.
    alerts: { telegram: true },
    requiresLiveData: false,
    riskNote: "A short's loss is unbounded — the stop is the whole risk control, not a formality. Most stocks cannot be held short overnight.",
    criteria: [
      { id: "brkdn", name: "Breakdown", enabled: true, checks: [
        { metric: "belowLow20", op: "gte", value: 1 },
        { metric: "dayChgPct", op: "lte", value: -2 },
        { metric: "pctFrom52wHigh", op: "gte", value: 15 },
      ] },
      { id: "distrib", name: "Distribution", enabled: true, checks: [
        { metric: "volMultiple", op: "gte", value: 2.5 },
      ] },
      { id: "trendbrk", name: "Trend broken", enabled: true, checks: [
        { metric: "belowDma50", op: "gte", value: 1 },
      ] },
    ],
  },
};
...
/* Trading around a core holding. Both are evaluated ONLY for symbols in
   holdings — `appliesTo` is what stops a holdings SELL being confused with a
   short. A holdings SELL trims something owned; the `short` profile above is the
   separate, opt-in thing that sells what you do not own. */
...
export function migrate(config) {
  if (config?.profiles && Object.keys(config.profiles).length) {
    ...
    return { ...structuredClone(HOLDING_PROFILES), ...structuredClone(SHORT_PROFILE), ...config.profiles };
  }
  const seeded = { ...structuredClone(DEFAULT_PROFILES), ...structuredClone(HOLDING_PROFILES), ...structuredClone(SHORT_PROFILE) };
...
/* Profiles the SCREENER runs. `appliesTo: "holdings"` profiles are deliberately
   excluded: they are evaluated only against stock the user already owns, by
   scanCycles. Running them over the universe would emit a SELL for a stock that
   is not held — a short recommendation, which this app does not make — and, more
   quietly, would file those results in the track record under the wrong direction.
   The flag was declared with the holdings profiles and never enforced; this is
   where it becomes real. */
export const enabledProfiles = profiles =>
  Object.entries(profiles).filter(([, p]) => p.enabled !== false && p.appliesTo !== "holdings");

/** The holdings-only profiles, for the cycle scan. The other half of the split. */
export const holdingsProfiles = profiles =>
  Object.entries(profiles).filter(([, p]) => p.enabled !== false && p.appliesTo === "holdings");
