...
export function status() {
  ...
  return {
    ...
    hint: !state.lastError ? null
      : /not found|404/i.test(state.lastError)
        ? `Repo or branch not found. Check DATA_REPO is exactly "owner/name", that the repo exists, and — for a fine-grained token — that this repo is listed under "Only select repositories".`
      : /403/.test(state.lastError)
        ? "Token rejected. A fine-grained PAT needs Repository permissions → Contents: Read and write. Check it has not expired."
      : /401/.test(state.lastError)
        ? "Token invalid or expired. Generate a new fine-grained PAT and update DATA_REPO_TOKEN."
      : /409|422/.test(state.lastError)
        ? "Write conflict — something else changed the file. This retries itself; if it persists, the branch may be protected."
        : null,
    detail: stale
