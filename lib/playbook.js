...
export function build(stock, { profile, dataAge, event, triggerPrice, direction = "buy", intent } = {}) {
  ...
  /* A structural stop can land inside the noise: the nearest support cluster was
     0.17% below the entry on one intraday row, which is not an invalidation level,
     it is a guaranteed exit on the next tick. The chart level is kept when it is
     far enough to mean something and widened to half an ATR when it is not — the
     zone still names where the idea breaks; the trigger just is not inside the
     spread. */
  const atrPctOfPrice = basisPrice > 0 ? (atrValue / basisPrice) * 100 : 0;
  /* Absolute as well as relative: on a near-zero-ATR stock half an ATR is also
     near zero, and the measured run still produced 0.05% stops. Below half a
     percent a stop is hit by the spread rather than by the setup failing. */
  const minStopPct = Math.max(0.5, atrPctOfPrice * 0.5);
  const rawStopPct = stopZ ? round2(pct(basisPrice, stopZ.mid))
    : round2((isSell ? 1 : -1) * atrPctOfPrice * 1.5);
  const stopPct = Math.abs(rawStopPct) < minStopPct && minStopPct > 0
    ? round2((isSell ? 1 : -1) * minStopPct)
    : rawStopPct;
  const stopWidened = stopPct !== rawStopPct;
  ...
  const exits = {
    ...
    stop: stopZ
      ? { zone: stopZ.zone, mid: stopZ.mid, pct: stopPct,
          ...
          rationale: isSell
            ? `Above ₹${stopZ.mid}, where ${stopZ.convergence} method${stopZ.convergence === 1 ? "" : "s"} place resistance — if it clears that, the trim was early and buying back gets expensive.${stopWidened ? " Widened to half an ATR: the cluster sat inside the day's noise, where a stop is hit by nothing happening." : ""}`
            : `Below ₹${stopZ.mid}, where ${stopZ.convergence} method${stopZ.convergence === 1 ? "" : "s"} place support — the level that would prove the idea wrong, not an arbitrary percentage.${stopWidened ? " Widened to half an ATR: the cluster sat inside the day's noise, where a stop is hit by nothing happening." : ""}` }
      : { zone: isSell
  ...
  /* Same rule the analog path applies, for the same reason: a level nobody
     should act on is worse than no level. Tested against the stretch, because
     the stop here is also structural-or-1.5×ATR — tail-sized — so judging it
     against the median condemns setups that a wider exit plan would justify.
     Without this the structure path was the remaining way a sub-1:1 target
     could still reach an alert. */
  const noRoom = exits.riskReward.toStretch != null && exits.riskReward.toStretch < 1;
  /* Always present, both values. Set only on the true branch, the canonical
     nested payload returned `undefined` while the compact rows built from it
     returned `false` — the same concept reading as "absent" on one path and
     "no" on the other, which is the bug this field was added to fix. */
  exits.noRoom = noRoom;
  if (noRoom) {
    exits.withheld = { safe: safe?.mid ?? null, primary: primary?.mid ?? null,
                       stretch: stretch?.mid ?? null, why: "below 1:1 against the stop" };
    exits.safe = null; exits.primary = null; exits.stretch = null;
    exits.riskRewardWarning = `Even the best-case target is ${exits.riskReward.toStretch}:1 against the stop. No target is offered — there is less to gain than to lose on any exit plan.`;
  } else {
    exits.riskRewardWarning = exits.riskReward.toPrimary != null && exits.riskReward.toPrimary < 1
      ? `Risk-reward to the primary target is ${exits.riskReward.toPrimary}:1 — only the stretch target (${exits.riskReward.toStretch}:1) pays for this stop.` : null;
  }
  ...
  /* Read from `exits`, not from the local zones the withholding did not touch.
     This block was built from `primary`/`safe`/`stretch` directly, so a noRoom row
     still published toPrimaryPct, magnitudePct and fromSpotToPrimaryPct — live
     percentages describing a target the engine had just refused to offer. BEL
     intraday shipped 3.1. A withheld level must be withheld everywhere it is
     visible, or it is not withheld; one decision, and every derived number falls
     out of it. */
  const pSafe = exits.safe, pPrimary = exits.primary, pStretch = exits.stretch;
  const potential = {
    direction,
    // Magnitude, so a sort on "potential" ranks by size regardless of direction.
    magnitudePct: pPrimary ? round2(Math.abs(pPrimary.pct)) : null,
    downward: isSell,
    toSafePct: pSafe?.pct ?? null, toPrimaryPct: pPrimary?.pct ?? null, toStretchPct: pStretch?.pct ?? null,
    movedAlreadyPct, exhausted: (pPrimary?.pct ?? 1) <= 0,
    // What every percentage above is measured from, stated rather than inferred.
    basisPrice: round2(basisPrice), basis: basisLabel,
    fromSpotToPrimaryPct: pPrimary ? round2(pct(price, pPrimary.mid)) : null,
    // Same conclusion, on the block a caller sorting by upside actually reads.
    noRoom: !!exits.noRoom,
  };
  ...
  return {
    ...
    reading: readingFor({ stock, entryZone, primaryZ, potential, entryConfidence, chasing, triggered, analysts,
                          noRoom: exits.noRoom, riskReward: exits.riskReward }),
  };
}

function readingFor({ stock, entryZone, primaryZ, potential, entryConfidence, chasing, triggered, analysts, noRoom, riskReward }) {
  const bits = [];
  ...
  /* Prose is rendered verbatim, so it must not name a level the payload withheld.
     This sentence read "Primary target ₹X–Y … N% away" straight off the zone,
     which the withholding never touched — the reading described a target the
     numbers beside it refused to give. */
  if (noRoom) {
    bits.push(`A target was worked out and rejected: even the best case is ${riskReward?.toStretch ?? "under 1"}:1 against the stop, so there is less to gain than to lose on any exit plan. That is the finding, not a gap.`);
  } else if (!primaryZ || primaryZ.convergence === 0) {
