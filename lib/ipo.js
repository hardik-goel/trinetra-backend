...
export async function expertCalls() {
  ...
  /* Reachability is per ISSUE, not per source, and the difference is not academic.
     Google News rate-limits under repeated querying, so a route routinely answers
     2 of 4 queries. Source-level `blocked` then computes false — one route did
     answer — and every issue reads as "checked, no expert view", including the
     issues where in fact no route answered at all. That is the exact conflation
     this endpoint exists to prevent, surviving one level down.

     When Pravesh publishes per-IPO discovery state, that is authoritative. Until
     then a partially-answering route means the certainty is `partial`: some issues
     were checked and some could not be, and which is which is not knowable from
     source-level counts. Reported as unknown rather than guessed per issue. */
  /* expert_reachability is the single authoritative source from schema 3 on.
     expert_coverage.perIpo is deliberately NOT read as a fallback — Pravesh chose
     one source over two that can drift, and honouring that means not quietly
     reintroducing the second. */
  const perIpo = {};
  for (const e of (j.expert_reachability || [])) {
    const slug = e.ipoSlug ?? e.ipo_slug;
    if (!slug) continue;
    const reachable = !!e.reachable;
    const checked = e.expertsChecked ?? null;
    const total = e.expertsTotal ?? null;
    const partial = reachable && Number.isFinite(checked) && Number.isFinite(total) && checked < total;
    perIpo[slug] = {
      ipoName: e.ipoName ?? e.ipo_name ?? null,
      reachable, expertsChecked: checked, expertsTotal: total,
      /* Three states per issue, not two. "partly" is the case where one expert
         answered and the other never did — a view from half the panel, which is
         not the same as the panel having no view. */
      status: !reachable ? "unchecked" : partial ? "partly" : "checked",
      meaning: !reachable
        ? "no route answered about this issue — the absence of an expert view here carries no information"
        : partial
          ? `${checked} of ${total} experts could be checked — the ones that went unchecked may or may not have had a view`
          : "every expert was checked — an absence of a view here is a real absence",
      routes: e.routes ?? null,
    };
  }
  const havePerIpo = Object.keys(perIpo).length > 0;

  // Roll-up, so a caller does not have to count the map itself.
  const issues = Object.values(perIpo);
  const perIpoSummary = havePerIpo ? {
    total: issues.length,
    checked: issues.filter(i => i.status === "checked").length,
    partly: issues.filter(i => i.status === "partly").length,
    unchecked: issues.filter(i => i.status === "unchecked").length,
  } : null;

  const partial = !blocked && allRoutes.some(r =>
    Number.isFinite(r.attempted) && Number.isFinite(r.answered) && r.answered < r.attempted);

  const certainty = blocked ? "none" : havePerIpo ? "per-issue" : partial ? "partial" : "full";

  return {
    ok: true, schemaVersion, supported: true,
    calls, sourceStatus,
    coverage: j.expert_coverage ?? null,
    /* "full"      — every route answered every query; an empty result is a real
                     absence of expert views.
       "per-issue" — Pravesh published reachability per IPO; use perIpo[slug] and
                     ignore the source-level flags entirely.
       "partial"   — some queries went unanswered. An empty result for a given
                     issue may mean nobody had a view, or may mean nobody was
                     asked, and source-level counts cannot tell you which.
       "none"      — nothing answered; the result says nothing at all. */
    certainty,
    perIpo: havePerIpo ? perIpo : null,
    perIpoSummary,
    /* The honest headline. Silence has two causes and they mean opposite things
       to someone deciding whether to apply. */
    blocked,
    verdict: perIpoSummary
      ? (perIpoSummary.unchecked === perIpoSummary.total
          ? `could not check any of the ${perIpoSummary.total} issues — no route answered about any of them`
          : `${perIpoSummary.checked} of ${perIpoSummary.total} issues fully checked, ${perIpoSummary.partly} partly, ${perIpoSummary.unchecked} not checked at all — only the checked ones can be said to have no expert view`)
      : blocked
      ? "could not check — every route was blocked or timed out, so the absence of expert calls says nothing"
      : calls.length === 0
        ? (certainty === "partial"
            ? "partly checked — some queries went unanswered, so for some issues the absence of a view means nobody was asked rather than nobody had one"
            : "checked, no attributable view — the routes that answered carried no explicit stance from these experts")
        : `${calls.length} attributable call${calls.length === 1 ? "" : "s"}${certainty === "partial" ? ", and some queries went unanswered — issues without a call here were not necessarily checked" : ""}`,
    ...
    limits: [
      ...
      "Reachability is per issue. Source-level counts cannot say which issues went unchecked when a route answers only some queries.",
    ],
  };
}
