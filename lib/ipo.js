...
export async function stats(days = 365) {
  ...
  const verdictOf = r => String(r.verdict ?? r.call ?? "").toLowerCase();
  const positive = skipped.filter(r => /apply|subscribe|green/.test(verdictOf(r)));
  ...
  out.skipped = {
    ...
    // The honest framing: this is what the engine's own calls did without you.
    note: "Listing gains on IPOs the engine called positively that you did not log an application for.",
  };
  return out;
}
