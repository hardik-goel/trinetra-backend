...
/* Called after an account is created, set by index.js. A new user needs the
   default criteria and a starting universe before their first scan, and doing it
   here means it happens at signup rather than at the next restart — otherwise a
   friend signs up, sees an empty dashboard, and concludes the app is broken. */
let onUserCreated = async () => {};
export const setUserCreatedHook = fn => { onUserCreated = fn; };
...
export function authRouter() {
  ...
  r.post("/auth/signup", guardDb, async (req, res) => {
    ...
    try {
      ...
      /* Seeded before the session is issued, so the first request after signup
         already has criteria to evaluate. A failure is logged and does not fail
         the signup — the account exists either way, and refusing it here would
         leave a user who cannot sign up and cannot sign in. */
      try { await onUserCreated(out.user); }
      catch (e) { console.warn(`[auth] seeding ${out.user.email} failed: ${e.message}`); }
