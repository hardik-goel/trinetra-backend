...
export function update(id, patch = {}) {
  ...
  if (patch.status === "closed" || patch.exitPrice != null) {
    ...
    t.realisedPnl = round2((exitPrice - t.entryPrice) * t.qty);
    t.realisedPct = round2(pct(t.entryPrice, exitPrice));
    t.mtm = null;
  }
  persist();
  return t;
...
export function markToMarket(priceBySymbol) {
  for (const t of trades) {
    ...
    t.mtm = {
      price: p,
      unrealisedPnl: round2((p - t.entryPrice) * t.qty),
      unrealisedPct: round2(pct(t.entryPrice, p)),
      stopHit: t.stopLoss != null && p <= t.stopLoss,
      targetHit: t.target != null && p >= t.target,
    };
  }
  return trades;
}
...
export function stats(days = 30, signals = [], horizon = 7) {
  ...
  const inWindow = trades.filter(t => Date.parse(t.entryDate) >= since);
